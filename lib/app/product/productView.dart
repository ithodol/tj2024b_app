...
class _ProductViewState extends State<ProductView>{ // [자식위젯]
  ...
  final dio = Dio();
  final baseUrl = "http://192.168.40.45:8080"; // 환경에 따라 변경

  bool isOwner = false; // 현재 로그인된 회원이 등록한 제품인지 확인
  ...
  void onView( ) async{
    try{ // * 직계 상위 부모 위젯의 접근 : widget.필드명  , widget.메소드명()
      ...
      if( response.data != null ){
        ...
        if(response2.data['memail'] == response.data['memail']){ // 회원정보의 아이디 == 제품 등록 회원아이디
          setState(() {
            isOwner = true;
          });
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("제품 상세 정보", style: TextStyle(color: Colors.white)),
      ...
      body: SingleChildScrollView(  // 내용이 길어지면 스크롤 제공하는 위젯
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox( height: 18,),
            Text( "${NumberFormat('#,###').format(product['pprice'])}원" , style: TextStyle( fontSize: 22 , fontWeight: FontWeight.bold , color: Colors.red ), ) ,
            SizedBox( height: 10 ,),
            Divider(), // 구분선 : vs <hr/>
            SizedBox( height: 10,),
            Row( // 가로 배치
            ...
            // 만약 isOwner가 true이면 로그인된 회원의 제품임
            if(isOwner == true)
              Row(
                children: [
                  ElevatedButton(
