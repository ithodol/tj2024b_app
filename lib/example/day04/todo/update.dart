...
class _UpdateSate extends State<Update> { // 클래스명 앞에 _ 언더바는 dart에서 private 키워드
  ...
  void todoFindById( int id ) async {
    try{
      final response = await dio.get("http://192.168.40.45:8080/day04/todos/view?id=$id");
  ...
  void todoUpdate(  ) async{
    try{
      ...
      final response =  await dio.put("http://192.168.40.45:8080/day04/todos" , data : sendData );
      final data = response.data;
